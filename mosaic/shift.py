...
def shift_manual(args):
    ...
    for itile in range(grid.shape[1]):
        ...
        dirPath = os.path.dirname(args.tmp_file_name)
        if not os.path.exists(dirPath):
            os.makedirs(dirPath)
        f = dx.File(args.tmp_file_name, mode='w') 
    ...
    os.system(f'tomocupy recon --file-type double_fov --binning {args.binning} --reconstruction-type try --file-name {args.tmp_file_name} \
    ...
    for jtile in range(1,grid.shape[1]):        
        ...
        for ishift,err_shift in enumerate(arr_err):
            ...
            for itile in range(grid.shape[1]):
                ...
                data,flat,dark,theta = dxchange.read_aps_32id(grid[0,::-step][itile],proj=(0,1))       
        ...
        dir = os.path.dirname(args.tmp_file_name)
        basename = os.path.basename(args.tmp_file_name)
        if not os.path.exists(dirPath):
            os.makedirs(dirPath)
        ...
        os.system(f'tomocupy recon --file-type double_fov --binning {args.binning} --reconstruction-type full \
            --file-name {args.tmp_file_name} --rotation-axis-auto manual --rotation-axis {center} --nsino-per-chunk {args.nsino_per_chunk}')            
